...
public abstract class GUI{
	...
	public enum Move {
		NORTH, SOUTH, EAST, WEST, ZOOM_IN, ZOOM_OUT, ROTATE_LEFT
	};
	...
	protected abstract void findShortestPath(String by);
	...
	public boolean UPDATE_ON_EVERY_CHARACTER = true;
	...
	protected JButton art;
	//protected JButton articulation;
	...
	private void initialise() {
		...
		JButton findPath = new JButton("Fastest route by distance");
		...
		JButton bytime = new JButton("Fastest route by time");
		bytime.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ev) {
				findShortestPath("time");
				redraw();
			}
		});
		
		art = new JButton("Find Articulation Points");
		...
		j = new JComboBox();
		j.setEditable(true);
		search = (JTextField) j.getEditor().getEditorComponent();
		...
		JPanel astar;
		JPanel articulation;
		astar = new JPanel(new GridLayout(2, 2));
		astar.setMaximumSize((new Dimension(105, 60)));
		astar.add(findPath);
		articulation = new JPanel(new CardLayout());
		articulation.setMaximumSize((new Dimension(105, 60)));
		articulation.add(art);
		//astar_bytime = new JPanel(new CardLayout());
		//astar_bytime.setMaximumSize(new Dimension(100,60));
		//astar_bytime.add(bytime);
		astar.add(bytime);
		controls.add(astar);
		//controls.add(astar_bytime);
		controls.add(articulation);
		...
		drawing.addMouseListener(new MouseAdapter() {
			 ...
			 public void mousePressed(MouseEvent e) {
				 ...
				 // store the point where the mouse was clicked
		        }
			 
			public void mouseReleased(MouseEvent e) {
				// store the position wheere the mouse was released
				// then find the distance between the two
				//back to comp 102 wire drawing for this i guess
				onClick(e);
				redraw();
			}
		});
		...
		frame = new JFrame("Auckland RoadMap by Mayank Chander");
